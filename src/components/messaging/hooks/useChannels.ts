 ...
 * Only owner can post, subscribers read.
 */
...
function mapChannelMessage(row: Record<string, unknown>): ChannelMessage {
  return {
    id: row.id as string,
...
export function useChannels(userId: string | undefined) {
  ...
  // Load channels I'm subscribed to
  const loadSubscribed = useCallback(async () => {
    if (!userId) return;
    const { data: subs } = await (supabase
      .from('channel_subscribers' as any)
      .select('channel_id') as any)
      .eq('user_id', userId);
  ...
  const subscribe = useCallback(async (channelId: string) => {
    ...
    await (supabase.rpc('increment_subscriber_count' as any, { cid: channelId }) as any).catch(() => {});
  ...
  const postMessage = useCallback(async (channelId: string, content: string, mediaUrl?: string, mediaType?: string) => {
    ...
    const msg = mapChannelMessage(data as Record<string, unknown>);
    setMessages(prev => [...prev, msg]);
    // Update channel last activity
    await (supabase
      .from('channels' as any)
      .update({ updated_at: new Date().toISOString() } as any) as any)
      .eq('id', channelId);
  }, [userId]);
  ...
  const loadMessages = useCallback(async (channelId: string) => {
    ...
    const { data } = await (supabase
      ...
      .limit(200);
    if (data) setMessages((data as any[]).map(mapChannelMessage));
    setLoading(false);
  }, []);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('ch-msgs:' + activeChannel.id)
      .on('postgres_changes', {
        event: 'INSERT',
        schema: 'public',
        ...
        const msg = mapChannelMessage(payload.new as Record<string, unknown>);
        setMessages(prev => {
  ...
  useEffect(() => {
    loadMyChannels();
    loadSubscribed();
  }, [loadMyChannels, loadSubscribed]);

  // Browse public channels
  const browsePublic = useCallback(async (query?: string) => {
    let q = (supabase.from('channels' as any).select('*') as any).eq('is_public', true).order('subscriber_count', { ascending: false }).limit(50);
