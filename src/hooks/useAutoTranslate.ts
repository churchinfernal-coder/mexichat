...
const LANG_PATTERNS: Array<{ lang: string; label: string; pattern: RegExp }> = [
  { lang: 'es', label: 'Espanol', pattern: /\b(que|por|para|como|esta|pero|con|una|los|las|del|hay|ser|mas|sin|sobre|tiene|puede|hace|este|donde|quien|cuando|porque|entre|desde|hasta|tambien|siempre|nunca|todos|nada|algo|mucho|poco)\b/gi },
  { lang: 'en', label: 'English', pattern: /\b(the|and|you|that|was|for|are|but|not|with|this|have|from|they|been|said|each|which|their|will|other|about|many|then|them|would|make|like|just|over|such|also|back|after|into|could|only|come|its|than|first|long|very|when|what|your|there)\b/gi },
...
function detectLanguage(text: string): { lang: string; label: string; confidence: number } {
  ...
  for (const { lang, label, pattern } of LANG_PATTERNS) {
    ...
    if (score > bestScore) { bestScore = score; bestLang = lang; bestLabel = label; }
  }
...
export function useAutoTranslate(userLang = 'es') {
  ...
  const translateMessage = useCallback(async (msgId: string, text: string) => {
    ...
    try {
      ...
      const result: TranslationResult = {
        ...
        detectedLang: detected.lang, targetLang: userLang,
      };
  ...
  return {
    shouldOfferTranslation, translateMessage, removeTranslation,
    getTranslation, isTranslating, getDetectedLanguage,
  };
}